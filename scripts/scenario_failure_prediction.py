...
def load_feature_table(suite_dir: Path) -> list[dict]:
    """Build a conversation-level feature table from test_set and scores."""
    ...
    test_set = {s["test_case_id"]: s for s in load_jsonl(suite_dir / "test_set.jsonl")}
    rows = []
...
def bootstrap_ci(test_case_ids: list[str], test_case_preds: dict[str, float], test_case_labels: dict[str, list[int]],
    ...
    unique_seeds = list(set(test_case_ids))
...
def test_case_texts(rows: list[dict]) -> dict[str, dict]:
    ...
    out = {}
    for r in rows:
        if r["test_case_id"] not in out:
            out[r["test_case_id"]] = {k: r[k] for k in ("title", "description", "system_prompt", "behavior", "definition")}
    return out


def loo_cv_predictions(test_case_ids: list[str], rows: list[dict], predict_fn) -> dict[str, float]:
    """Leave-one-seed-out CV. predict_fn(train_rows, test_test_case_id) -> float."""
    unique_seeds = sorted(set(test_case_ids))
    preds = {}
...
def evaluate_predictions(preds: dict[str, float], labels: dict[str, list[int]], test_case_ids: list[str],
    ...
    ci = bootstrap_ci(test_case_ids, preds, labels, n_boot=n_boot)
...
def stage0(rows: list[dict]) -> dict:
    ...
    print("\n=== Stage 0: Scenario Determinism ===")
    print(f"Test Set: {result['n_seeds']}, Conversations: {result['n_conversations']}")
    print(f"Base rate: {base_rate:.3f}")
    print(f"Deterministic test_set: {n_deterministic}/{len(rates)} ({result['pct_deterministic']:.0%})")
    print("Per-seed failure distribution:")
    for k, v in sorted(dist.items()):
        print(f"  {k}: {v} test_set")
...
def stage1_baselines(rows: list[dict], n_boot: int = 2000) -> dict:
    ...
    preds_global = loo_cv_predictions(test_case_ids, rows, predict_global)
    results["global_base_rate"] = evaluate_predictions(preds_global, labels, test_case_ids, n_boot)

    # Behavior base rate
    def predict_behavior(train, test_sid):
        test_sr = next(r["behavior"] for r in rows if r["test_case_id"] == test_sid)
        same_sr = [r for r in train if r["behavior"] == test_sr]
        if not same_sr:
            return sum(r["policy_violation"] for r in train) / len(train)
        return sum(r["policy_violation"] for r in same_sr) / len(same_sr)
    preds_sr = loo_cv_predictions(test_case_ids, rows, predict_behavior)
    results["behavior_base_rate"] = evaluate_predictions(preds_sr, labels, test_case_ids, n_boot)
...
def stage1_embedding_baselines(rows: list[dict], embeddings: dict[str, list[float]], n_boot: int = 2000) -> dict:
    ...
    results["embedding_nn"] = evaluate_predictions(preds_nn, labels, test_case_ids, n_boot)
    ...
    preds_lr = loo_cv_predictions(test_case_ids, rows, predict_lr)
    results["logistic_regression"] = evaluate_predictions(preds_lr, labels, test_case_ids, n_boot)
...
def robustness_checks(rows: list[dict], secondary_rows: list[dict],
    ...
    for r in rows:
        if r["test_case_id"] not in behavior_seeds[r["behavior"]]:
            behavior_seeds[r["behavior"]].append(r["test_case_id"])
    ...
    results["within_behavior"] = {
        ...
        "n_behavior_categories": len(within_aurocs),
