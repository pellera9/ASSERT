...
from p2m.stages.systematization import run, run_systematization
...
class SystematizationStageTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_uses_risk_name_when_concept_missing(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            with patch("p2m.stages.systematization.generate_structured", new=fake_generate_structured):
                result = await run(ctx, {"mode": "direct"})

        self.assertEqual(result["systematization_path"], str((root / "systematization.json").resolve()))
        self.assertIn("# Concept Label\nharmful advice", str(captured["messages"]))
        self.assertFalse(bool(captured["web_search"]))
        self.assertIsNone(captured["reasoning_effort"])

    async def test_run_rejects_reasoning_effort_in_direct_mode(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            ctx = {
                "suite_root": root,
            ...
            with self.assertRaisesRegex(ValueError, "only allowed when mode=research"):
                await run(ctx, {"mode": "direct", "reasoning_effort": "high"})
