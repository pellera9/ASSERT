...
def run_pipeline(
    *,
    ...
    # so external observers see live progress during long stages (rollout
...
def _run_stages_inner(
    *,
    ...
    for stage_name, module, raw_cfg in stages_to_run:
        ...
        # so a stage that doesn't report progress (e.g. policy, design)
        # doesn't leave stale {"stage": "rollout", "completed": 1000}
        # in the manifest.
        if watchdog is not None:
            watchdog.tick()
        if heartbeat is not None:
